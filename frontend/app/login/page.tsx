...
import { AlertTriangle, Facebook, Eye, EyeOff } from 'lucide-react';
import AuthLayout from '@/components/AuthLayout';
import Image from 'next/image';

export default function LoginPage() {
    const router = useRouter();
    const [form, setForm] = useState({ email: 'chaitanyasai2947@gmail.com', password: 'Sai@123456' });
    ...
    return (
        <AuthLayout>
            <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
                <h1 style={{ fontWeight: 800, fontSize: '2.2rem', letterSpacing: '-0.02em', color: 'white' }}>Login Now</h1>
            </div>

            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                    <input className="input-field" type="email" required placeholder="Email or Username"
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                    <input className="input-field" type={showPassword ? 'text' : 'password'} required placeholder="Password"
                        style={{ background: '#F3F8FF', border: 'none', borderRadius: '6px', padding: '1rem', paddingRight: '3rem', width: '100%', color: '#043873', fontSize: '0.95rem' }}
                        value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} style={{ position: 'absolute', right: '1rem', background: 'transparent', border: 'none', color: '#64748B', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '1.5rem', marginBottom: '1.5rem' }}>
                <p style={{ color: '#043873', fontSize: '0.85rem', fontWeight: 700, marginBottom: '1rem' }}>Or login with</p>
            ...
            <p style={{ textAlign: 'center', marginTop: '2rem', color: '#043873', fontSize: '0.95rem', fontWeight: 700 }}>
                Not a member?{' '}
                <Link href="/register" style={{ color: '#FFB800', textDecoration: 'none' }}>Signup now</Link>
            </p>
            <p style={{ textAlign: 'center', marginTop: '0.5rem', fontSize: '0.85rem' }}>
                <Link href="/forgot-password" style={{ color: '#E4F0FD', textDecoration: 'none', opacity: 0.9 }}>Forgot Password?</Link>
            </p>
        </AuthLayout>
    );
}
